    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1E1B17]/60 backdrop-blur-xs p-4">
      <div className="bg-[#FBF8F1] border border-[#E7E1D3] rounded-2xl p-6 max-w-md w-full shadow-lg relative space-y-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-base font-bold font-heading text-[#1E1B17]">Explain to a Friend</h3>
            <p className="text-xs text-[#6E6659]">One-tap plain-language share card</p>
          </div>
        </div>
        ...
        <div className="p-4 bg-[#F6F1E7] rounded-xl border border-[#E7E1D3] space-y-2 text-xs">
          ...
          <div className="p-2.5 bg-[#FBF8F1] rounded text-[#1E1B17] font-medium border border-[#E7E1D3]">
            Consequence: {clause.one_line_consequence}
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 pt-1">
          <button
            onClick={handleCopy}
            className="flex-1 py-2.5 px-4 bg-[#B85C38] hover:bg-[#9C4B2B] text-white text-xs font-bold rounded-lg shadow-xs flex items-center justify-center space-x-1.5 transition-colors"
          >
            {copied ? <Check className="w-4 h-4 text-white" /> : <Copy className="w-4 h-4" />}
            <span>{copied ? 'Copied to Clipboard' : 'Copy Share Card'}</span>
          </button>
        </div>
      </div>
    </div>
